...
def log_emotion(npc_name: str, emotion: str, label: str):
    """记录当前情绪"""
    dialogue_logger.info(f"😊 当前情绪: {label} ({emotion})")

def log_emotion_change(npc_name: str, old_emotion: str, new_emotion: str, old_label: str, new_label: str):
    """记录情绪变化"""
    dialogue_logger.info(f"🔄 情绪变化: {old_label} -> {new_label}")

def log_analyzing_affinity():
    """记录正在分析好感度"""
    dialogue_logger.info("📊 正在分析好感度与情绪...")
...
# 启动时记录日志文件位置（避免 Windows GBK 控制台无法输出 emoji）
print(f"\n[日志] 对话日志文件: {LOG_FILE}")
print(f"[日志] 日志目录: {LOGS_DIR}\n")
